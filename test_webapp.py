...
try:
    ...
    check("HTML loads CSS", 'css/style.css' in html)
    check("HTML loads sentence.js", 'js/sentence.js' in html)
    check("HTML loads app.js", 'js/app.js' in html)
...
try:
    with open("frontend/css/style.css") as f:
        css = f.read()
    check("CSS has dark background", "#0b0c0e" in css or "#0f0f1a" in css)
    ...
    check("CSS has responsive breakpoint", "@media" in css and "max-width" in css)
